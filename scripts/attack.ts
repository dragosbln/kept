...
const ATTACKS_DIR = path.join(import.meta.dirname, 'attacks');
...
async function loadScript(nameOrPath: string): Promise<AttackScript> {
  const candidates = [
...
async function loadAllScripts(): Promise<AttackScript[]> {
